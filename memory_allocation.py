...
def add_to_memory_firstfit(event_list,process_id,arrival_time,burst_time,process_size,total_size,curr_time,memory_allocated,wait_queue):
        ...
        if not memory_allocated:
            if(process_size < total_size):
                ...
                event_list.append(process_id,0,curr_time+burst_time,burst_time,process_size)
                new_list = sorted(event_list,key=lambda x: (x[2],x[1]))
                event_list = new_list
            else:
                new_pair1 = (process_id,process_size)
                wait_queue.append(new_pair1)     
            temp_memory['memory_state'] = memory_allocated
            temp_memory['processes_waiting'] = wait_queue
            return temp_memory
            # find free memory block 
        for pair in memory_allocated:    
                process_name1,start1,end1 = pair
                #only last pair left to check
                if(i == memory_allocated.size()-1):
...
def remove_from_memory_firstfit(process_id,end_time,process_size,total_size,curr_time,memory_allocated,wait_queue):
        temp_memory = {}
...
def first_fit(data):
    ...
    for event in event_list:
        process_id,arrival_bit,arrival_time,burst_time,process_size = pair
        if(arrival_bit == 1):
            ...
            memory_chart += temp_memory
        else:
            #process has completed its execution
            #here arrival time=curr_time+burst_time set by add_to_memory function
            end_time = arrival_time
            temp_memory = remove_from_memory_firstfit(process_id,end_time,burst_time,process_size,total_size,curr_time,memory_allocated,wait_queue)
            temp_memory['event'] = event
            memory_chart += temp_memory
    return memory_chart
...
def add_to_memory_worstfit(event_list,process_id,arrival_time,burst_time,process_size,total_size,curr_time,memory_allocated,wait_queue):
        ...
        for pair in memory_allocated:    
                ...
                process_name2,start2,end2 = memory_allocated[i+1]
                i = i+1
                #if memory available, allocate largest slot of unallocated memory
                if(start2-end1 >= process_size):
...
def worst_fit(data):
    ...
    for event in event_list:
        ...
        if(arrival_bit == 1):
            ...
            #here arrival time=curr_time+burst_time set by add_to_memory function
            end_time = arrival_time
            temp_memory = remove_from_memory_worstfit(process_id,end_time,burst_time,process_size,total_size,curr_time,memory_allocated,wait_queue)
            temp_memory['event'] = event
            memory_chart +=temp_memory
    return memory_chart
